        ...
        <nav className="flex flex-col gap-2">
          {links.map((link) => (
            <SidebarLink 
              key={link.label} 
              link={link}
              className="group/link relative overflow-hidden flex justify-center"
            />
          ))}
          <button 
            onClick={handleLogout}
            className="flex items-center justify-center group/link py-3 px-4 rounded-lg hover:bg-neutral-100/80 dark:hover:bg-neutral-800/80 transition-all duration-200 w-full"
          >
